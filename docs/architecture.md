...
The Manager is configured with custom rules (`netflow_rules.xml`) that act on the flat JSON the collector sends. Decoding is handled by the Wazuh built in JSON decoder rather than by a custom decoder, which the sample alerts confirm: every one of them carries `"decoder": {"name": "json"}`. A `netflow_decoder.xml` is included in the repository as a starting point for future customization, and it is not required for this pipeline to work.

**Recommended specs:**
* 4 vCPU, 8 GB RAM, 50 GB disk
* Ubuntu 22.04 or CentOS 8+

## VM 2, Linux Agent and NetFlow Collector
...
2. **Python normalization script**. Reads the raw pmacctd output and converts it into flat JSON that the built in Wazuh JSON decoder can parse. Fields are renamed with an `nf_` prefix, timestamps are standardized, and flow duration is calculated.
