...
def base_event(src_ip: str, dst_ip: str, src_port: int, dst_port: int,
    ...
    return {
        ...
        "source":      {"ip": src_ip, "port": str(src_port)},
        "destination": {"ip": dst_ip, "port": str(dst_port)},
        "flow":        {"protocol": proto, "direction": direction},
        "network":     {"bytes": bytes_, "packets": packets},
        "event":       {"duration": round(duration, 3), "type": "network", "category": "network_traffic"},
        "tcp":         {"flags": flags},
        "internal":    {"src": direction.startswith("internal"), "dst": direction.endswith("internal")},
